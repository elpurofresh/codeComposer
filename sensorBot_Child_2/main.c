...
void txPacket(char *buffer)
{
	...
	for (i = 0; i < 10; ++i) {
		...
		for (j = 0; j < buffLen; ++j) {  // Send 20 times the data packet with a 0.25sec pause in between
...
void parsePacket(void)
{
	sprintf(tsec1, "%c%c", uartBuffer[4], uartBuffer[5]);
	tsec2 = atoi(tsec1);
	if (tsec2 >= 60) {
		tsec2 = tsec2 - 60;
	}
	...
	sprintf(tmin1, "%c%c", uartBuffer[6], uartBuffer[7]);
	tmin2 = atoi(tmin1);
	if (tmin2 >= 60) {
		tmin2 = tmin2 - 60;
	}
	...
	sprintf(thrs1, "%c%c", uartBuffer[8], uartBuffer[9]);
	thrs2 = atoi(thrs1);
	hrs = thrs2 - 24;

	sprintf(tPH1, "%c%c", uartBuffer[20], uartBuffer[21]);
	tPH2 = atoi(tPH1);
	tPH2 = tPH2 - 14;
