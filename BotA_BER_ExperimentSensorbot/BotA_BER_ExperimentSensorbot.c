...
char keyCode[KEYCODESIZE] 	= "<REQTCX>\0";
char dataVal[KEYCODESIZE-3]	= "GC424\0";

volatile int dataLengthOut = 0;
char uartRxBuff[BUFF_SIZE] 	= "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@\0";  //includes brackets [<-data->]
char uart_Out[BUFF_SIZE+1] 	= "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@\0"; //includes ending char '!'
...
volatile int masterRequest = FALSE;
...
__interrupt void USCI_A0_ISR(void)
{
	...
	case 2:                                   // Vector 2 - RXIFG
		//test[dataLengthIn] = UCA0RXBUF;
		uartRxBuff[dataLengthIn++] = UCA0RXBUF;


		if (UCA0RXBUF == keyCode[keyCounter]) {
			keyCounter++;
			if (keyCounter == (KEYCODESIZE-1)) {
				keyCounter = 0;
				masterRequest = TRUE;
			}
		}
