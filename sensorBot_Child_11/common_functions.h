...
void initPorts(void);						//initializes Ports
...
void UART_SlaveWrite (const char *buf);
...
void turnSlave(int val) {

	if (val == ON) {
		PJOUT |= BIT1;

		P1DIR |= SlaveEn_0 + SlaveEn_1;
		P1OUT |= SlaveEn_0 + SlaveEn_1;
		sleepQuart();
	}
	else if (val == OFF) {
		P1OUT &= ~(SlaveEn_0 + SlaveEn_1);
...
void UART_SlaveWrite (const char *buf)
{
	...
	//P1OUT |= BIT0 + BIT1;   // Turn on switch to allow connection
