...
void init_I2C(unsigned char slave_address);
...
void init_I2C(unsigned char slave_address)
{
	...
	UCB0CTL1  &= ~UCSWRST;
	UCB0IE    |= UCTXIE0 + UCRXIE0 + UCNACKIE; 	// Enable TX and RX ready and No ACK interrupts
...
void resetTime(unsigned char *newTime)
{
	...
	while ( TI_USCI_I2C_notready() );	// wait for bus to be free
	// initialization
	transmitI2C(1, newTime);			// start transmitting
}
